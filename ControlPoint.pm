...
sub new {
	...
	# Create the socket on which search requests go out
    	$self->{_searchSocket} = IO::Socket::INET->new(Proto => 'udp', Reuse => 1,  LocalPort => $searchPort) ||
	...
	# subscribed.
    	$self->{_subscriptionSocket} = HTTP::Daemon->new( Reuse => 1, LocalPort => $subscriptionPort) || croak("Error creating subscription socket: $!\n");
	...
	$self->{_ssdpMulticastSocket} = IO::Socket::INET->new(	 Proto => 'udp',
								 Reuse => 1,
								 LocalPort => SSDP_PORT) ||
...
sub _createDevice {
	...
	if ($response->is_success) {
		($device, $base) = $self->parseDeviceDescription($response->content,{Location => $location},{ControlPoint => $self});
	}
	else {
...
sub _receiveSSDPEvent {
	...
	if ($method ne 'NOTIFY') {
		...
		#print $method," request ignored.\n";
		return;
	}

	my $headers = UPnP::Common::parseHTTPHeaders($buf);
	my $eventType = $headers->header('NTS');
	my $device = $self->_getDeviceFromHeaders($headers, $eventType =~ /alive/ ?  1 : 0);
...
sub _receiveSubscriptionNotification {
	...
	if ($request && ($request->method eq 'NOTIFY') &&
		...
		if ($subscription) {
			my %propSet = $self->_parsePropertySet($content);
...
sub subscribe {
	...
	if (defined($cp)) {
		...
		if ($response->is_success &&
			...
			my $subscription = UPnP::ControlPoint::Subscription->new(  Service => $self,
										   Callback => $callback,
										   SID => $sid,
										   AutoRenew => defined($autorenew) ? $autorenew : 1,
										   Timeout => $timeout,
										   EventSubURL => "$url");
	...
	unless (defined($som->fault())) {
		for my $out ($args{Action}->outArguments) {
			...
			if ($data) {
				...
				$data->type($type);
				if ($type eq 'string') {
					$self->{_results}->{$name} = HTML::Entities::decode($data->value);
				}
				else {
...
sub _passesFilter {
	...
	if ((!defined($type) || index($device->deviceType(),$type) != -1 || 
		 ($type eq 'ssdp:all')) &&
...
sub autoRenew {
	my $self = shift;	
	if ($self->aboutToExpire() && $self->{_autoRenew}){
		print "Automatically renewed subscription.\n";
		$self->renew();
	}
...
sub aboutToExpire {
        ...
        if ($now - $self->{_startTime} > $self->{_timeout}*0.85) {
                return 1;
        }
